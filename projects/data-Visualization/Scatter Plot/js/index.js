...
d3.json(url, function (error, data) {
  ...
  */
  ...
  scaleTime().
  domain([
  d3.min(data, function (d) {return parseTime(d.Time);}),
  d3.max(data, function (d) {return parseTime(d.Time);})]).
  ...
  scaleTime().
  domain([
  d3.min(data, function (d) {return parseDate(d.Year - 1);}),
  d3.max(data, function (d) {return parseDate(d.Year + 1);})]).
  ...
  attr("data-xvalue", function (d, i) {return parseDate(d.Year);}).
  attr("data-yvalue", function (d, i) {return parseTime(d.Time);}).
  attr("cx", function (d, i) {return xscale(parseDate(d.Year));}).
  attr("cy", function (d, i) {return yscale(parseTime(d.Time));}).
  attr("r", 5);
  /*
  ...
  attr("x", "0.25em").
  attr("y", function (d, i) {return 0.25 + i * 1.5 + "em";}).
  attr("width", "1em").
  attr("height", "1em");
  ...
  append("text").
  attr("x", "1.5em").
  attr("y", function (d, i) {return 1.1 + i * 1.5 + "em";}).
  attr("font-size", "1em").
  ...
  function onMouseOver(d, i) {
    ...
    tooltip.html(function (d) {return data[i].Name + ": " + data[i].Nationality + "<br/>" + "Year: " + data[i].Year + ", Time: " + data[i].Time + (data[i].Doping ? "<br/><br/>" + data[i].Doping : "");}).
    attr("data-year", function (d) {return parseDate(data[i].Year);});
  }
